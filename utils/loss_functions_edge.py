...
# ==================== 기본 Segmentation Loss & 기존 KD Loss ====================
...
class FocalLoss(nn.Module):
    ...
    def forward(self, input, target):
        ...
        # one-hot target에서 class=1 채널만 사용 (2-class 기준)
...
def prediction_map_distillation(y, teacher_scores, T=4):
    ...
    :param T: for softmax temperature
    """
...
def importance_maps_distillation(s, t, exp=4):
    """
    Attention-based KD (AT)
    https://arxiv.org/abs/1612.03928
    """
    if s.shape[2:] != t.shape[2:]:
        s = F.interpolate(s, t.size()[-2:], mode='bilinear', align_corners=False)
...
def region_affinity_distillation(s, t, gt):
    ...
    :param gt: ground-truth mask (one-hot)
    """
